 ...
 * @author Gonnyh.I
 */
class ElementCollection extends yii\base\Model {
    ...
    function all() {
        ...
        $targets = \sinelnikof88\abac\models\TargetRule::find()->with('policy')->with(['policy', 'policy.policyElements', 'policy.policyElements.element'])->asArray()->where(['target_id' => $attr])->all();

        $classesNames = [];
        try {
            foreach ($targets as $policy) {
                if ($policy['policy']['policyElements'])
                    foreach ($policy['policy']['policyElements'] as $rules) {

                        $classesNames[] = $rules['element']['class'];
                    }
            }
        } catch (Exception $exc) {
